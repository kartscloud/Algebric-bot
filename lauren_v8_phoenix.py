...
"""
================================================================================
LAUREN v8 PHOENIX - Risen From The Ashes
================================================================================

🔥 WHAT'S NEW IN v8 PHOENIX:
- Fixed signal generation (v7_flip was blocking everything)
- Relaxed filters to catch 16+ signals vs 0 before
- IV spike check now only applies to LONG (HIGH IV = good for SHORT!)
- Kept all the good exit rules from v7_flip

CHANGES FROM v7_FLIP:
- MIN_VOLUME: 50 → 10 (was blocking 348 options!)
- MIN_OI: 50 → 10
- MIN_SCORE_STRENGTH: 0.15 → 0.10
- DTE_MAX: 10 → 14
- MAX_OTM: 12% → 20%
- MIN_CONFIDENCE: 60 → 55
- IV_SPIKE: Now LONG-only (SHORT loves high IV!)

EXIT RULES (unchanged - these work great):
┌─────────────────────────────────────────────────────────────────────┐
...
MIN_SCORE_STRENGTH = 0.10  # Score must be at least 0.10 away from threshold
                           # LONG needs score < -0.40, SHORT needs > 0.50
                           # Relaxed from 0.15 to catch more signals
...
MAX_IV_SPIKE = 2.00        # For LONG only: skip if IV > 200% of HV
                           # (HIGH IV on LONG = buying overpriced)
                           # Note: This check does NOT apply to SHORT signals
                           # (HIGH IV on SHORT = GOOD - that's why it's overpriced!)
...
DTE_MAX = 14         # Max 14 days (extended from 10 to catch more signals)
MIN_OTM = 0.02       # Min 2% OTM
MAX_OTM = 0.20       # Max 20% OTM (extended from 12% to match aggro)
MIN_PREMIUM = 0.10   # Min $0.10 option price
MIN_BID = 0.05       # Min $0.05 bid
MIN_OI = 10          # Min 10 open interest (relaxed - was blocking signals)
MIN_VOLUME = 10      # Min 10 volume (relaxed - was blocking 348 options!)
...
MIN_CONFIDENCE = 55  # Min 55% confidence (relaxed from 60%)
...
DATA_DIR = "lauren_v8_data"
LOG_DIR = "lauren_v8_logs"
...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler(f'{LOG_DIR}/phoenix.log')
    ]
)
...
def scan_for_signals(opts: List[Dict], spots: Dict[str, float], macro: Dict, rh=None) -> List[Dict]:
    ...
    for o in opts:
        ...
        if raw_score < LONG_THRESH:
            ...
            # ANTI-CHASE: For LONG only, check IV spike
            # (For SHORT, high IV is GOOD - that's what makes it overpriced!)
            if iv and hv and iv > hv * MAX_IV_SPIKE:
                logger.debug(f"Skip {ticker} ${strike}: IV spike for LONG ({iv:.0%} > {hv*MAX_IV_SPIKE:.0%})")
                continue
...
║     ╚══════╝╚═╝  ╚═╝ ╚═════╝ ╚═╝  ╚═╝╚══════╝╚═╝  ╚═══╝                      ║
║                                                                              ║
║     🔥 v8 PHOENIX - RISEN FROM THE ASHES 🔥                                  ║
║                                                                              ║
║     Fixed signal generation + Smart exit rules                               ║
║     Stop Loss: -35% | Trail: +40% activate, 25% distance                     ║
                ...
                if signals:
                    ...
                    if not already_have:
                        ...
                        if contracts >= 1 and contract_cost <= trader.lauren_buying_power:
                            ...
                            # Log position with correct exit rules
                            sl_price = actual_entry * (1 + STOP/100)  # -35%
                            trail_activate_price = actual_entry * (1 + TRAIL_ACTIVATE/100)  # +40%
                            logger.info(f"")
                            logger.info(f"   📊 POSITION OPENED:")
                            logger.info(f"   Entry: ${actual_entry:.2f}")
                            logger.info(f"   Stop Loss: ${sl_price:.2f} ({STOP}%)")
                            logger.info(f"   Trail Activates: ${trail_activate_price:.2f} (+{TRAIL_ACTIVATE}%)")
                            logger.info(f"   Trail Distance: {TRAIL_DISTANCE}%")
...
if __name__ == "__main__":
    ...
    if '--trades' in sys.argv:
        ...
        for t in trades:
            ...
            opt_sym = 'P' if t.get('direction') == 'SHORT' else 'C'
            print(f"{t['date']} | {t['ticker']} ${t['strike']}{opt_sym} | {t['pnl_pct']:+.0f}% | ${t['pnl_dollar']:+.2f} {w}")
        if trades:
