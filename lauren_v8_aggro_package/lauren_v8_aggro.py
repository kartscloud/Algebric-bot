...
"""
================================================================================
LAUREN v8 AGGRO - Realistic Fills + Aggressive Sizing
================================================================================

The best of both worlds:
- v8's realistic fill modeling (entry slippage, bid-anchored exits)
- v8's circuit breakers and safety features
- v7's aggressive position sizing (but with a max loss cap)

Config Philosophy:
- Max 50% of capital per trade (not 100% YOLO)
- Max $500 loss per trade (not unlimited)
- Realistic slippage on entries
- Bid-anchored exits
- All circuit breakers active

Expected Results:
- Higher returns than conservative v8
- Lower risk than fantasy v7
...
MIN_BUYING_POWER_RESERVE = 50.0    # Keep $50 reserve (minimal)
...
MIN_PREMIUM = 0.10
MIN_BID = 0.05
MIN_OI = 25                        # Relaxed from v8's 50
MAX_SPREAD_PCT = 0.15              # Relaxed from v8's 12%
...
# =============================================================================
# CIRCUIT BREAKERS (Keep these - they save you)
# =============================================================================

MARKET_OPEN_DELAY_MINUTES = 5      # Reduced from 10
MAX_CONSECUTIVE_REJECTS = 5        # More tolerance
MAX_SPREAD_BLOWOUT_PCT = 0.30      # Wider tolerance
MIN_QUOTE_FRESHNESS_SEC = 120      # More tolerance
...
DATA_DIR = "lauren_v8_aggro_data"
LOG_DIR = "lauren_v8_aggro_logs"
...
logging.basicConfig(
    level=logging.INFO,
    format='%(asctime)s | LAUREN-AGGRO | %(levelname)s | %(message)s',
    handlers=[
...
<html>
<head>
    <title>LAUREN AGGRO - Live Logs</title>
...
def calculate_entry_fill(
    ...
    Higher confidence / lower liquidity = more slippage.
    """
    # Confidence factor
    if confidence >= 90:
        conf_slip = 0.50
...
def calculate_exit_fill(bid: float, ask: float, urgency: str) -> float:
    ...
    if urgency == 'stop':
        return round(bid + (mid - bid) * 0.15, 2)  # Very close to bid
    elif urgency == 'time':
...
def calculate_position_size(
    ...
    """
    AGGRO sizing: 50% of capital, capped at max loss.
    """
    if entry_price <= 0:
        return 0
    ...
    # Method 1: Max loss cap
    max_by_risk = int(MAX_LOSS_PER_TRADE / cost_per_contract)
    
    # Method 2: Percentage of capital
    available = (buying_power - MIN_BUYING_POWER_RESERVE) * MAX_POSITION_PCT
    if available <= 0:
        return 0
    max_by_pct = int(available / cost_per_contract)
    
    # Take the LARGER of the two (AGGRO mode)
    # But never more than we can afford
    max_affordable = int((buying_power - MIN_BUYING_POWER_RESERVE) / cost_per_contract)
    
    contracts = min(max(max_by_risk, max_by_pct), max_affordable)
...
class LaurenTrader:
    ...
    def buy_option(self, ticker: str, strike: float, exp: str, 
        ...
        logger.info(f"🎯 BUY {quantity} {ticker} ${strike}C @ ${limit_price:.2f}")
    ...
    def sell_option(self, ticker: str, strike: float, exp: str,
                    quantity: int, limit_price: float) -> Dict:
        logger.info(f"💰 SELL {quantity} {ticker} ${strike}C @ ${limit_price:.2f}")
...
def scan_for_signals(trader: LaurenTrader, opts: List[Dict], macro: Dict) -> List[Dict]:
    signals = []
...
║     ╚══════╝╚═╝  ╚═╝ ╚═════╝ ╚═╝  ╚═╝╚══════╝╚═╝  ╚═══╝                      ║
║                                                                              ║
║     v8 AGGRO - Realistic Fills + Aggressive Sizing                           ║
║     Position: 50% capital | Max Loss: $500/trade                             ║
║     TP: +50% | SL: -40% | Circuit Breakers: ON                               ║
║                                                                              ║
╚══════════════════════════════════════════════════════════════════════════════╝
    """)
    ...
    logger.info(f"📊 Position Size: {MAX_POSITION_PCT*100:.0f}% | Max Loss: ${MAX_LOSS_PER_TRADE}")
    ...
    while True:
        try:
            ...
            if state['realized_pnl'] <= -MAX_DAILY_LOSS:
                logger.info(f"🛑 Daily loss limit: ${state['realized_pnl']:.2f}")
                update_state(done_for_day=True)
                continue
            ...
            for pos in positions[:]:
                ...
                if should_exit:
                    logger.info(f"")
                    logger.info(f"{'='*60}")
                    logger.info(f"📤 EXIT: {exit_reason} | {pos['ticker']} ${pos['strike']}C")
            ...
            if len(positions) < MAX_POSITIONS:
                macro = {'trend': 'NEUTRAL'}
                signals = scan_for_signals(trader, opts, macro)
                ...
                if signals:
                    ...
                    if not safe:
                        ...
                        logger.info(f"")
                        logger.info(f"{'='*60}")
                        logger.info(f"📊 SIGNAL: {best['direction']} {best['ticker']} ${best['strike']}C")
...
if __name__ == "__main__":
    ...
    if '--trades' in sys.argv:
        ...
        for t in trades:
            ...
            print(f"{t['date'][:10]} | {t['ticker']} ${t['strike']}C | "
