...
Based on v6 autonomous + v3 signal thresholds (which actually worked)

What's new in v7 AGGRO:
- 100% buying power (no more splitting with quant_trader)
- v3 signal thresholds restored (LONG < -0.30, SHORT > 0.40)
- SHORT signals RE-ENABLED (made 66% avg in backtest)
- More aggressive position sizing
- Relaxed filters to catch more opportunities

Backtest Results (December 2025):
- v6: 4 trades, 50% win rate, -1.7% compound (TOO STRICT)
- v7: 6 trades, 100% win rate, +3,610% compound (LETS GO)
...
- Smart entry pricing (v6's fix for ASK slippage)

Targets: NVDL, TSLL (leveraged single-stock ETFs)
Buying Power: 100% FULL SEND
...
MAX_POSITION_PCT = 0.50    # 50% per position (AGGRO)
...
STOP = -35           # Stop loss at -35% (was -40%, tighter now)
                        ...
                        if contracts >= 1 and contract_cost <= trader.lauren_buying_power:
                            ...
                            for attempt in range(max_attempts):
                                if attempt == 0:
                                    ...
                                    T = best['dte'] / 365
                                    hv = HIST_VOL.get(best['ticker'], 0.65)
